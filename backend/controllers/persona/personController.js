...
// Función para obtener opciones de Sistema de Salud
exports.getRefHealthInsuranceCoverage = async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query('SELECT * FROM RefHealthInsuranceCoverage');
    res.status(200).json(result.recordset);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// Función para obtener opciones Enfermedad
exports.getRefIDEADisabilityType = async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query('SELECT * FROM RefDisabilityType');
    res.status(200).json(result.recordset);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
exports.createPerson = async (req, res) => {
  ...
  try {
    ...
    const {FirstName, SecondName, MiddleName, LastName, Birthdate, BirthdateVerification, RefSexId, RefPersonalInformationVerificationId, 
      RefTribalAffiliationId, RefVisaTypeId, RUT, RefPersonStatusTypeId, RefStateId, RefCountyId, TelephoneNumber, 
      TelephoneNumberType, MobileNumber, MobileNumberType ,StreetNumberAndName,EmailAddress,ReactionDescription
      ,RefDisabilityTypeId,RefHealthInsuranceCoverageId} = req.body;

    console.log('Request Body:', req.body); // Para depuración
    console.log('RefDisabilityTypeId:', RefDisabilityTypeId); // Para depuración
    ...
    //  .input('RefPersonalInformationVerificationId', sql.Int, RefPersonalInformationVerificationId)  //Aqui colocar 1
    //  .input('RefTribalAffiliationId', sql.Int, RefTribalAffiliationId)    //Aqui colocar 1
    //  .input('RefVisaTypeId', sql.Int, RefVisaTypeId)                      //Aqui colocar 1

        .input('RefPersonalInformationVerificationId', sql.Int, 1)  //Aqui colocar 1
        .input('RefTribalAffiliationId', sql.Int, 1)    //Aqui colocar 1
        .input('RefVisaTypeId', sql.Int, 1)                      //Aqui colocar 1
    ...
    await requestAddress
      ...
      .input('StreetNumberAndName', sql.NVarChar, StreetNumberAndName)
    ...
    // Insertar el Email  en la tabla PersonEmailAddress
    await requestEmail 
      .input('PersonId', sql.Int, PersonId)
      .input('EmailAddress', sql.NVarChar, EmailAddress)
      .input('RefEmailTypeId', sql.Int, 1) // Asumiendo que el ID de sistema de identificación es 1      
      .query(`INSERT INTO PersonEmailAddress(PersonId, EmailAddress, RefEmailTypeId)
              VALUES (@PersonId, @EmailAddress, @RefEmailTypeId)`);


// Insertar Alergico a
    const requestReactionDescription  = new sql.Request(transaction);
    // Insertar el Alergico a   en la tabla PersonAllergy
    await requestReactionDescription        
      .input('PersonId', sql.Int, PersonId)
      .input('RefAllergyTypeId', sql.Int, 1) // Asumiendo que el ID de sistema de identificación es 1  
      .input('RefAllergySeverityId', sql.Int, 1) // Asumiendo que el ID de sistema de identificación es 1     
      .input('ReactionDescription', sql.NVarChar,ReactionDescription)       
      .query(`INSERT INTO PersonAllergy(PersonId, RefAllergyTypeId, RefAllergySeverityId,ReactionDescription)
              VALUES (@PersonId, @RefAllergyTypeId, @RefAllergySeverityId,@ReactionDescription)`);


// Insertar Sistema Salud              
   
    const requestRefHealthInsuranceCoverage = new sql.Request(transaction);
    // Insertar el Alergico a   en la tabla PersonHealth
    await requestRefHealthInsuranceCoverage       
      .input('PersonId', sql.Int, PersonId)
      .input('VisionScreeningDate',  sql.Date, new Date()) // Asumiendo que el ID de sistema de identificación es 1  
      .input('RefVisionScreeningStatusId', sql.Int, 1) // Asumiendo que el ID de sistema de identificación es 1     
      .input('HearingScreeningDate',  sql.Date, new Date()) 

      .input('RefHearingScreeningStatusId', sql.Int, 1) // Asumiendo que el ID de sistema de identificación es 1  
      .input('DentalScreeningDate', sql.Date, new Date() ) // Asumiendo que el ID de sistema de identificación es 1     
      .input('RefDentalScreeningStatusId', sql.Int, 1) 

      .input('RefHealthInsuranceCoverageId', sql.Int, RefHealthInsuranceCoverageId) // Nuevo Sistema de Salud ID 
      .input('RefDentalInsuranceCoverageTypeId', sql.Int, 1) // Asumiendo que el ID de sistema de identificación es 1     
      .input('RefMedicalAlertIndicatorId', sql.Int, 1) 
 
      .input('HealthScreeningEquipmentUsed', sql.NVarChar, ' ') // Asumiendo que el ID de sistema de identificación es 1     
      .input('HealthScreeningFollowUpRecommendation', sql.NVarChar, ' ') 
     
      .query(`INSERT INTO PersonHealth(PersonId,VisionScreeningDate,RefVisionScreeningStatusId,HearingScreeningDate,RefHearingScreeningStatusId
      ,DentalScreeningDate,RefDentalScreeningStatusId,RefHealthInsuranceCoverageId,RefDentalInsuranceCoverageTypeId,RefMedicalAlertIndicatorId
      ,HealthScreeningEquipmentUsed,HealthScreeningFollowUpRecommendation)
      VALUES (@PersonId,@VisionScreeningDate,@RefVisionScreeningStatusId,@HearingScreeningDate,@RefHearingScreeningStatusId
      ,@DentalScreeningDate,@RefDentalScreeningStatusId,@RefHealthInsuranceCoverageId,@RefDentalInsuranceCoverageTypeId,@RefMedicalAlertIndicatorId
      ,@HealthScreeningEquipmentUsed,@HealthScreeningFollowUpRecommendation)`);


// Insertar Enfermedad
const requestRefIDEADisabilityType = new sql.Request(transaction);

await requestRefIDEADisabilityType        
  .input('PersonId', sql.Int, PersonId)
  .input('PrimaryDisabilityTypeId', sql.Int, 1) // Ajusta si el ID no es 1
  .input('DisabilityStatus', sql.Bit, 1) // Ajusta si el ID no es 1     
  .input('RefAccommodationsNeededTypeId', sql.Int, 1) 
  .input('RefDisabilityConditionTypeId', sql.Int, 1)
  .input('RefDisabilityDeterminationSourceTypeId', sql.Int, 1) // Ajusta si el ID no es 1  
  .input('RefDisabilityConditionStatusCodeId', sql.Int, 1) // Ajusta si el ID no es 1     
  .input('RefIDEADisabilityTypeId', sql.Int, RefDisabilityTypeId) 
  .input('SignificantCognitiveDisabilityIndicator', sql.Bit, 1) 
  .query(`
