   ...
   // Agregar log para tipoMatricula
	...
	// Condicional para actualizar LivesWithIndicator
      ...
      if (viveCon === 'Apoderado') {
        ...
        console.log('Insertando teléfono apoderado...');
        ...
        await requestApoderadoPhone.query(`
          ...
          VALUES (@PersonId, @TelephoneNumber)
        `);
        console.log('Teléfono apoderado insertado.');

        console.log('Insertando email apoderado...');
        ...
        await requestApoderadoEmail.query(`
          ...
          VALUES (@PersonId, @EmailAddress)
        `);
        console.log('Email apoderado insertado.');

        console.log('Insertando relación apoderado-estudiante...');
		...
		requestTipoMatricula.input('RecordEndDateTime', sql.DateTime, fechaRetiro || null);
            ...
            // -----------------------------
            // EXISTENTE: Insertar en OrganizationPersonRole para nivel y curso
            // -----------------------------
            ...
            requestOrgPersonRole.input('OrganizationIdNivel', sql.Int, nivel);
            ...
            await requestOrgPersonRole.query(`
                ...
                VALUES (@PersonId, @OrganizationIdNivel, @RoleID),
                       (@PersonId, @OrganizationIdCurso, @RoleID)
            `);
		...
		if (fechaRetiro && fechaRetiro !== '')
            ...
            */
            ...
            if (runIpeValue && !usuarioExiste) {
                ...
                requestAuthEstudiante.input('SchoolId', sql.Int, nivel);
                ...
                try {
                    ...
                    `);
                    console.log('POST-INSERT UserAuthentication ESTUDIANTE: OK');
                } catch (err) {
                    console.error('ERROR en insert UserAuthentication ESTUDIANTE:', err);
                    throw err;
                }
                console.log('Estudiante insertado en UserAuthentication.');
            } else if (runIpeValue && usuarioExiste) {
                console.log('No se inserta UserAuthentication para el estudiante (ya existe).');
            }

            // Confirmar transacción
            console.log('Antes de commit de la transacción...');
            await transaction.commit();
            console.log('Transacción comprometida exitosamente.');

            // --- Insertar apoderado en UserAuthentication FUERA de la transacción principal ---
            if (viveCon === 'Apoderado' && rutOipeApoderado && rutOipeApoderadoTipo) {
                try {
                    ...
                    requestAuthApoderado.input('RoleId', sql.Int, 5);
                    requestAuthApoderado.input('SchoolId', sql.Int, nivel);
                    requestAuthApoderado.input('DeletedAt', sql.DateTime, null);
                    requestAuthApoderado.input('LastLogin', sql.DateTime, null);
                    console.log('PRE-INSERT UserAuthentication APODERADO (fuera de transacción):', {
                        UserName: rutOipeApoderado,
                        Password: '12345',
                        FirstName: primerNombreApoderado,
                        LastName: apellidoPaternoApoderado,
                        SecondLastName: apellidoMaternoApoderado,
                        Address: domicilioApoderado,
                        RoleId: 5,
                        SchoolId: nivel
                    });
                    ...
                    `);
                    console.log('POST-INSERT UserAuthentication APODERADO (fuera de transacción): OK');
                } catch (err) {
                    console.error('ERROR en insert UserAuthentication APODERADO (fuera de transacción):', err);
                    // Aquí puedes decidir si quieres notificar al frontend o solo loguear el error
                }
            }
            ...
            res.status(500).json({ error: 'Error creando el estudiante', details: error.message });
        }
