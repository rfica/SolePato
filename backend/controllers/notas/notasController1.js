  ...
  try {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      await new sql.Request(transaction)
        ...
        .input('RefAcademicSubjectId', sql.Int, 1)
        .input('RefAssessmentSubtestTypeId', sql.Int, 1) // Tipo Directa
        .query(`
          IF NOT EXISTS (
    ...
    for (const estudiante of estudiantes) {
      console.log(`[CREAR_HOJA] Evaluando estudiante ${estudiante.PersonId}`);
      ...
      const existe = await new sql.Request(transaction)
        ...
        .query(`
          ...
          WHERE PersonId = @PersonId
            AND CourseSectionOrganizationId = @cursoId
            AND OrganizationId = @asignaturaId
      ...
      console.log(`[CREAR_HOJA] Creando hoja para estudiante ${estudiante.PersonId}`);
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        const subForm = await new sql.Request(transaction)
          .input('Name', sql.NVarChar(40), shortName)
          .query(`SELECT AssessmentFormId FROM AssessmentForm WHERE Name = @Name`);
        const subFormId = subForm.recordset[0]?.AssessmentFormId;
        if (!subFormId) throw new Error(`[CREAR_HOJA] No se encontró AssessmentFormId para ${shortName}`);
        ...
        console.log(`[CREAR_HOJA] Insertando resultado ${shortName} para estudiante ${estudiante.PersonId}`);
