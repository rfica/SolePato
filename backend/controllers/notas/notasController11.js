...
exports.guardarNotasAcumuladas = async (req, res) => {
  try {
    ...
    for (const alumno of subnotas) {
      ...
      console.log(`[GUARDAR_ACUMULATIVA] Promedio acumulado: ${alumno.promedio}`);
        ...
        for (let i = 0; i < alumno.notas.length; i++) {
          ...
          if (nota !== null && nota !== undefined && !isNaN(nota)) {
            await request.query(`
              ...
              VALUES (${registrationId}, ${assessmentSubtestId}, ${nota}, 0, '${fecha}')
            `);
            console.log(`[GUARDAR_ACUMULATIVA] Subnota insertada para ${registrationId} - nota[${i}]: ${nota}`);
          } else {
      ...
      if (alumno.promedio !== null && alumno.promedio !== undefined && !isNaN(alumno.promedio)) {
        await request.query(`
          INSERT INTO AssessmentResult
          (AssessmentRegistrationId, AssessmentSubtestId, ScoreValue, IsAverage, DateCreated)
          VALUES (${registrationId}, ${assessmentSubtestId}, ${alumno.promedio}, 1, '${fecha}')
        `);
        console.log(`[GUARDAR_ACUMULATIVA] Promedio insertado para ${registrationId}: ${alumno.promedio}`);
      } else {
...
exports.getNotasAcumuladas = async (req, res) => {
  try {
    ...
    const columnasNumericas = columnas.map(col => {
      if (!isNaN(col)) return parseInt(col);
      const match = col.match(/N(\d+)/);
      return match ? parseInt(match[1]) : null;
    }).filter(id => id !== null);

    if (columnasNumericas.length === 0) {
      return res.status(400).json({ error: 'Columnas inválidas para búsqueda' });
    }
	
	console.log(`[GET_NOTAS_ACUMULADAS] Consultando con cursoId=${cursoId}, asignaturaId=${asignaturaId}, columnas=${columnasNumericas}`);
    ...
    const query = `
      SELECT 
        ...
        res.ScoreValue,
        res.IsAverage,
        ROW_NUMBER() OVER (PARTITION BY ar.AssessmentRegistrationId, res.IsAverage ORDER BY res.AssessmentResultId) - 1 AS NotaIndex
      FROM AssessmentResult res
      INNER JOIN AssessmentRegistration ar ON ar.AssessmentRegistrationId = res.AssessmentRegistrationId
      ...
      WHERE ar.CourseSectionOrganizationId = @cursoId
        ...
        AND res.AssessmentSubtestId IN (${columnasNumericas.join(',')})
    `;
