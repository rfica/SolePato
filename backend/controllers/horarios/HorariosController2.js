...
exports.saveBulkHorarios = async (req, res) => {
    ...
    try {
		...
		const nuevoId = insertResult.recordset[0].RecessScheduleId;

		bloque.scheduleId = nuevoId;
bloque.classMeetingDays = classMeetingDays;
bloque.classBeginningTime = classBeginningTime;
bloque.classEndingTime = classEndingTime;
bloque.classPeriod = classPeriod;
bloque.organizationId = calendarData.organizationId;
bloque.profesor = null;
bloque.tipoBloque = "Recreo";
bloque.bloqueId = bloque.bloqueId || null;
bloque.asistencia = false;


		console.log("✅ Recreo insertado con ID:", nuevoId);
  ...
  const nuevoId = insertResult.recordset[0].CourseSectionScheduleId;
...
bloque.classEndingTime = classEndingTime;
bloque.classPeriod = classPeriod;
bloque.organizationId = organizationId;
bloque.profesor = profesor;
bloque.tipoBloque = "Clase";
bloque.bloqueId = bloque.bloqueId || null;
bloque.asistencia = asistencia === true;



  console.log("✅ Clase insertada con ID:", nuevoId);
}
        }
