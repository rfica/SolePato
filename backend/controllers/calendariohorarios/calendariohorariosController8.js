...
exports.guardarObservacionCurso = async (req, res) => {
  try {
    ...
    // 🔍 Buscar solo por curso y bloque (sin considerar tipo de observación)
    const existeResult = await pool.request()
      .input('ScheduleId', sql.Int, scheduleId)
      .input('OrganizationId', sql.Int, organizationId)
      .query(`
        SELECT TOP 1 IncidentId, FileUrl
        FROM Incident
        WHERE CourseSectionScheduleId = @ScheduleId
    ...
    if (existeResult.recordset.length > 0) {
      const incidentId = existeResult.recordset[0].IncidentId;
      const fileAnterior = existeResult.recordset[0].FileUrl;

      const fileFinal = filename !== null ? filename : fileAnterior;

      // ✅ UPDATE
      await pool.request()
        .input('IncidentId', sql.Int, incidentId)
        .input('IncidentDescription', sql.NVarChar(sql.MAX), texto)
        .input('FileUrl', sql.NVarChar(500), fileFinal)
      ...
      console.log(`📝 Observación actualizada correctamente (ID: ${incidentId})`);
      return res.json({ message: 'Observación actualizada correctamente.' });
    }
    ...
    console.log(`🆕 Observación insertada correctamente (scheduleId: ${scheduleId})`);
