      ...
      <div className="modal-form">
        ...
        <div className="modal-row">
          ...
          <select className="form-control" value={evaluacion} onChange={e => setEvaluacion(e.target.value)}>
            ...
            {tiposEvaluacionRef.current.map(op => <option key={op.id} value={op.id}>{op.Description}</option>)}
          </select>
...
const Notas = () => {
  ...
  const handleBuscar = async () => {
    ...
    try {
      ...
      console.log("NotasGuardadas recibidas:", notasGuardadas);
	  
		if (
			  Array.isArray(notasGuardadas) &&
			  notasGuardadas.some(n => n.AssessmentSubtestId && n.ScoreValue !== undefined && n.OrganizationPersonRoleId)
			) {



        // Si existen notas, mostrar la tabla con los datos
        // Mapear las notas a la estructura de la tabla
        // Suponiendo que cada nota tiene OrganizationPersonRoleId, AssessmentSubtestId (1-10), ScoreValue
        // Agrupar por estudiante
		
					
				  // Paso 1: detectar nombres únicos de columnas
			const nombresColumnasUnicas = [...new Set(notasGuardadas.map(n => n.Identifier))].sort((a, b) => {
			  const n1 = parseInt(a.replace('N', '')) || 0;
			  const n2 = parseInt(b.replace('N', '')) || 0;
			  return n1 - n2;
			});

			// Paso 2: definir componentes dinámicamente
			setComponentes(nombresColumnasUnicas.map(n => ({ nombre: n, porcentaje: '' })));

			// Paso 3: construir la matriz de notas por estudiante
			const notasPorEstudiante = resEst.data.map(est => {
			  const filaNotas = {};
			  notasGuardadas
				.filter(n => n.OrganizationPersonRoleId === est.OrganizationPersonRoleId)
				.forEach(n => {
				  filaNotas[n.Identifier] = n.ScoreValue !== null ? n.ScoreValue : '';
				});
			  return filaNotas;
			});
        ...
        if (confirm.isConfirmed) {
          ...
          try {
            ...
            const assignedByPersonId = usuario?.personId || usuario?.PersonId;
					   ...
					   // Línea previa: const payload = {
			const payload = {
			  cursoId: parseInt(cursoSeleccionado),          // ahora cursoId será usado como OrganizationId (curso)
			  asignaturaId: parseInt(asignaturaSeleccionada), // ahora asignaturaId será CourseSectionOrganizationId
			  periodoId: parseInt(periodoSeleccionado),
			  assignedByPersonId,
			  estudiantes: resEst.data.map(e => ({ PersonId: e.PersonId }))
			};
            ...
            setNotas(resEst.data.map(() => Array(10).fill('')));
            setTipoNota('ACUMULATIVA');
          } catch (err) {
            Swal.close();
            Swal.fire({
          ...
          <table className="tabla-notas">
            <thead>
              <tr>
				...
				<th key={c.nombre} onClick={() => handleAbrirModalColumna(c.nombre)}>
               ...
               {componentes.map((c) => (
				  <td key={c.nombre}>
					<input
					  type="number"
					  step="0.1"
					  value={notas[idxEst]?.[c.nombre] || ''}
					  onChange={e => actualizarNota(idxEst, c.nombre, e.target.value)}
