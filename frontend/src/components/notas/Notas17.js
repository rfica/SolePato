          ...
          if (String(cambio.valorAnterior) !== String(cambio.valorNuevo)) {
            try {
              await axios.post('http://localhost:5000/api/notas/log-cambio-columna', {
                ...
                valorAnteriorDescripcion: cambio.mapearValor ? cambio.mapearValor(cambio.valorAnterior) : String(cambio.valorAnterior),
                valorNuevoDescripcion: cambio.mapearValor ? cambio.mapearValor(cambio.valorNuevo) : String(cambio.valorNuevo),
                usuarioId: usuario?.id || usuario?.personId || usuario?.PersonId || 1
              });
            } catch (logError) {
...
const Notas = () => {
  ...
  const handleVisualizacionColumna = async (colNombre, nuevoTipo) => {
    ...
    try {
      ...
      // Swal.fire('Error', 'No se pudo registrar el cambio', 'error');
    }
  };
