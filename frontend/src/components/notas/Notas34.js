...
const ModalConfiguracionNota = ({ visible, tipo, columna, onClose, escalas, tiposEvaluacion, visualizacionColumnas, cursoId, asignaturaId, assessmentId, configColumna, onGuardadoExitoso }) => {
         ...
         if (subnotasParaBackend.length > 0) {
           try {
               ...
               if (respuesta.data.success) {
                   ...
                   // CALLBACK AL PADRE: Avisar que el guardado fue exitoso
                   if (onGuardadoExitoso) {
                     console.log("[DEBUG] Llamando callback al padre después de guardar subnotas");
                     onGuardadoExitoso();
                   }
               } else {
                    Swal.fire({
             ...
             try {
               ...
               if (estudiantesParaRegistrar.length > 0) {
                 ...
                 if (respuesta.data && respuesta.data.registrosCreados > 0) {
                   ...
                   // CALLBACK AL PADRE: Avisar que la creación fue exitosa
                   if (onGuardadoExitoso) {
                     console.log("[DEBUG] Llamando callback al padre después de crear registros");
                     onGuardadoExitoso();
                   }
                 } else {
...
const Notas = () => {
  ...
  // CALLBACK: Función para recargar datos después del guardado exitoso
  const recargarDespuesDeGuardar = async () => {
    try {
      console.log("[DEBUG] Recargando datos de la tabla después del guardado exitoso...");
      
      // 1. Obtener estudiantes actualizados
      ...
      // 2. Consultar notas actualizadas
      ...
      console.log("[DEBUG] Datos recargados - Notas encontradas:", notasGuardadas.length);
      ...
      if (notasGuardadas && notasGuardadas.length > 0) {
        ...
        // Construir matriz de notas por estudiante y columna
        ...
        // Inicializar las notas para la visualización
        ...
        console.log("[DEBUG] Tabla actualizada exitosamente después del guardado");
      }
    } catch (err) {
      console.error("[ERROR] Error al recargar datos de la tabla:", err);
    }
  };
    ...
    <div className="notas-container">
      ...
      {tipoNota === 'ACUMULATIVA' && estudiantes.length > 0 && (
        <div className="card tabla">
          ...
          onGuardadoExitoso={recargarDespuesDeGuardar}
