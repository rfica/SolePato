...
const ModalConfiguracionNota = ({ visible, tipo, columna, onClose, escalas, tiposEvaluacion, visualizacionColumnas, cursoId, asignaturaId, assessmentId }) => {
  ...
  const [noInfluye, setNoInfluye] = useState(false);
		 ...
		 const cargarOAs = async () => {
		  try {
			...
			console.log("OA recibidos:", res.data); // 👈 AÑADE ESTA LÍNEA
  ...
  const handleAgregarOA = () => {
    ...
    if (oaObj) setOasAgregados([...oasAgregados, oaObj]);
  };
      ...
      <div className="modal-form">
        ...
        <div className="modal-row">
          ...
          >
            ...
            {tiposEvaluacion.map(op => (
              <option key={op.id} value={op.Description}>{op.Description}</option>
            ))}
          </select>
          <label style={{marginLeft: 'auto'}}>
            <input
              ...
              disabled={evaluacion === "Diagnóstica"}
        ...
        <div className="modal-row">
          ...
          <button
            ...
            disabled={!oaSeleccionado || oasAgregados.some(oa => String(oa.LearningObjectiveId) === String(oaSeleccionado))}
            style={{ marginLeft: 8 }}
          >
            + Agregar
          </button>
            ...
            <div style={{ width: '100%' }}>
              {oasAgregados.map(oa => (
                <div key={oa.LearningObjectiveId} style={{ display: 'flex', alignItems: 'center', marginBottom: 4, background: '#f4f8ff', borderRadius: 4, padding: '2px 8px' }}>
                  <span style={{ flex: 1 }}>{oa.Code}: {oa.Description}</span>
                  <button
