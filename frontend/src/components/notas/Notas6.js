    ...
    contenido = (
      <div className="modal-form">
        ...
        <div className="modal-row">
          ...
          <textarea className="form-control" defaultValue="Prueba escrita de divisiones" />
        </div>
        <div className="modal-row">
          <label>Tipo de nota {columna}:</label>
          <label><input type="radio" name="tipoNota" defaultChecked /> Directa</label>
    ...
    try {
      ...
      if (notasGuardadas && notasGuardadas.length > 0) {
        ...
        const notasPorEstudiante = resEst.data.map(est => {
          const notasEst = Array(10).fill('');
          notasGuardadas.filter(n => n.OrganizationPersonRoleId === est.OrganizationPersonRoleId).forEach(nota => {
            if (nota.AssessmentSubtestId >= 1 && nota.AssessmentSubtestId <= 10) {
              notasEst[nota.AssessmentSubtestId - 1] = nota.ScoreValue !== null ? nota.ScoreValue : '';
            }
          });
          return notasEst;
        });
