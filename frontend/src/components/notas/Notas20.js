...
const ModalConfiguracionNota = ({ visible, tipo, columna, onClose, escalas, tiposEvaluacion, visualizacionColumnas, cursoId, asignaturaId, assessmentId, configColumna }) => {
	  ...
	  const actualizados = subnotas.map(alumno => {
		...
		for (let i = 0; i < alumno.notas.length; i++) {
		  ...
		  if (!isNaN(nota) && !isNaN(peso)) {
			suma += nota * (peso / totalPeso);
		  }
		}
		return {
		  ...alumno,
		  promedio: parseFloat(suma.toFixed(2))
		};
	  });

	  setSubnotas(actualizados);
	}, [subnotas.map(s => s.notas), subnotas.map(s => s.pesos)]);
  ...
  const cargarNotasAcumuladas = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/notas/notas-acumuladas', {
        cursoId,
        asignaturaId,
  ...
  const handleGuardarConfiguracion = async () => {
    ...
    if (parseInt(tipoColumna) === 2 && notasAcumuladas.length > 0) {
      const payload = {
        assessmentSubtestId: assessmentId,
        fecha: fechaEvaluacion,
        subnotas: notasAcumuladas.map(alumno => ({
          assessmentRegistrationId: alumno.assessmentRegistrationId,
          notas: alumno.notas,
          pesos: alumno.pesos,
      ...
      await axios.post('http://localhost:5000/api/notas/notas-acumuladas', payload);
      console.log('[DEBUG] Subnotas acumulativas enviadas:', payload);
    }


      // Registrar cambios en el log (solo si hay configuración previa)
      if (configColumna && assessmentId) {
        ...
        // Registrar solo los cambios que realmente cambiaron
          ...
          if (String(cambio.valorAnterior) !== String(cambio.valorNuevo)) {
            try {
              ...
              await axios.post('http://localhost:5000/api/notas/log-cambio-columna', {
				  assessmentId,
				  //assessmentSubtestId: componentes.find(c => c.nombre === nombreColumna)?.id, // 👈 ESTE CAMPO ES CLAVE
				  assessmentSubtestId: subtests.find(c => c.nombre === nombreColumna)?.id,
				  identifier: nombreColumna,
				  ...
				  valorNuevoDescripcion: nuevoValor
				});
                  ...
                  <td key={j}>
                    <input
                      ...
                      onChange={e => {
                        ...
                        nuevas[i].notas[j] = parseFloat(e.target.value);
                        setSubnotas(nuevas);
                      }}
                    />
                  </td>
                ))}
                <td style={{ color: 'blue' }}>{alumno.promedio?.toFixed(1) || '0.0'}</td>
              </tr>
            ))}
          </tbody>
        </table>
