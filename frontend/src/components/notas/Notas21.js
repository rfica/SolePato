...
const ModalConfiguracionNota = ({ visible, tipo, columna, onClose, escalas, tiposEvaluacion, visualizacionColumnas, cursoId, asignaturaId, assessmentId, configColumna }) => {
  ...
  useEffect(() => {
    if (configColumna) {
	  ...
	  console.log('[DEBUG] Modal abierto con tipoColumna:', tipoColumna, 'y columna:', columna);
...
useEffect(() => {
  if (visible && assessmentId && columna) {
    console.log(`[CONFIGURACION_EXISTENTE] Buscando configuración para ${columna}...`);
  ...
  const cargarNotasAcumuladas = async () => {
    if (tipoColumna !== 2 || columnasOrigen.length === 0 || !cursoId || !asignaturaId) return;
    ...
    try {
      ...
      console.log("[DEBUG FRONTEND] Notas acumuladas:", res.data);
...
}, [tipoColumna, columnasOrigen, cursoId, asignaturaId]);
    ...
    await axios.post('http://localhost:5000/api/notas/configurar-columna', {
     ...
     console.log('[DEBUG] Intentando guardar configuración acumulativa con subnotas:', subnotas);
					...
					if (parseInt(tipoColumna) === 2 && subnotas.length > 0) {
					  ...
					  const payload = {
						assessmentSubtestId: assessmentId,
						fecha: fechaEvaluacion,
...
const confirmarCambioTipo = async (nuevoValor) => {
  ...
  if (parseInt(nuevoValor) === 2) {
    ...
    if (resultado.isConfirmed) {
      try {
        // Crear registros AssessmentRegistration para estudiantes faltantes
        const estudiantes = subnotas.map(est => ({
        ...
        const res = await axios.post('http://localhost:5000/api/notas/crear-assessment-registrations', {
          ...
          estudiantes
						...
						// Refrescar assessmentRegistrationId para todos los estudiantes después de crear los registros faltantes
				try {
				  const refresco = await axios.post('http://localhost:5000/api/notas/obtener-assessment-registrations', {
					assessmentId: assessmentId,
					organizationPersonRoleIds: subnotas.map(e => e.organizationPersonRoleId)
				  });

				  const registrosActualizados = refresco.data;

				  // Actualizar subnotas con los nuevos AssessmentRegistrationId
				  setSubnotas(prev =>
					prev.map(est => {
					  const registro = registrosActualizados.find(r => r.OrganizationPersonRoleId === est.organizationPersonRoleId);
					  return registro
						? { ...est, assessmentRegistrationId: registro.AssessmentRegistrationId }
						: est;
					})
				  );

				  console.log('[INFO] Refresco exitoso de assessmentRegistrationId después de creación');
				} catch (error) {
				  console.error('[ERROR] Falló refresco de registros después de creación:', error);
				}
        ...
        console.error('[ERROR] al crear registros en AssessmentRegistration', err);
        Swal.fire('Error', 'No se pudieron preparar los registros de estudiantes.', 'error');
        return;
      }

      // Cambiar a tipo acumulativa y limpiar subnotas previas
      setTipoColumna(2);
      setSubnotas([]);
	  
	  
				  // Actualizar registrationId después de crear registros
			try {
			  ...
			  const nuevos = subnotas.map(al => {
				const match = actualizados.find(x => x.OrganizationPersonRoleId === al.organizationPersonRoleId);
				return {
				  ...al,
				  assessmentRegistrationId: match?.AssessmentRegistrationId || null
				};
			  });
			  setSubnotas(nuevos);
			  console.log('[DEBUG] subnotas actualizadas con registrationId:', nuevos);
			} catch (e) {
			  console.error('[ERROR] al actualizar assessmentRegistrationId tras crear:', e);
			}
    ...
    <div className="modal-form">
      ...
      <div className="modal-row">
        ...
        <select className="form-control" value={evaluacion} onChange={e => setEvaluacion(e.target.value)}>
          <option value="">Seleccione</option>
          {tiposEvaluacion.map(op => <option key={op.id} value={op.id}>{op.Description}</option>)}
        </select>
        <label style={{ marginLeft: 'auto' }}>
...
const Notas = () => {
  ...
  const handleVisualizacionColumna = async (colNombre, nuevoTipo) => {
    ...
    // Buscar el AssessmentSubtestId de la columna
    const columnaObj = componentes.find(c => c.nombre === colNombre);
    const assessmentSubtestId = columnaObj?.id;

    if (!assessmentSubtestId) {
      console.error('No se encontró AssessmentSubtestId para la columna', colNombre);
      return;
    }
    ...
    try {
      ...
      setVisualizacionColumnas(prev => ({ ...prev, [colNombre]: nuevoTipo }));
	   ...
	   // Obtener usuario autenticado
		  ...
		  if (!usuarioId) {
			console.error('[ERROR] Usuario no autenticado al intentar registrar visualización');
      ...
      console.error('Error al registrar cambio de visualización:', error);
      // Revertir el cambio en el estado local si hay error
      setVisualizacionColumnas(prev => ({ ...prev, [colNombre]: valorAnterior }));
    }
  };
  ...
  const confirmarCambioTipo = (nuevoTipo) => {
    if (parseInt(nuevoTipo) === 2 && tipoColumna !== 2) {
      Swal.fire({
        icon: 'warning',
        title: 'Atención',
        text: 'Al continuar, esta N se transformará en "Acumulativa", por lo tanto, se perderán todas las notas que hayan sido ingresadas anteriormente, ¿Desea continuar?',
      ...
      setTipoColumna(parseInt(nuevoTipo));
    }
  };
