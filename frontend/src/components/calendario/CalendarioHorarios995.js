...
if (!cursoId || cursoId === 'No disponible') {
  setMensajeObs('Error: El ID del curso no está definido correctamente.');
  return;
}
...
const handleEditarObservacion = (obs) => {
  Swal.fire({
    ...
    preConfirm: async () => {
      ...
      try {
        await axios.put(`http://localhost:5000/api/calendariohorarios/observaciones/curso/${obs.Id}`, {
          texto: nuevoTexto,
