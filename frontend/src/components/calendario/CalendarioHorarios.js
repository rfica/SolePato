...
const [observacionClase, setObservacionClase] = useState('');
const [tipoClase, setTipoClase] = useState('1');
...
const handleFirmarBloque = async () => {
  setLoadingFirma(true);

  if (!horario?.scheduleId) {
    alert('⛔ Falta información del bloque para poder firmar.');
    setLoadingFirma(false);
    return;
  }

  // Obtener la fecha y hora actual con formato exacto "YYYY-MM-DDTHH:mm:ss-04:00"
  const ahora = new Date();
  const pad = (n) => n.toString().padStart(2, '0');
  const fechaFormateada = `${ahora.getFullYear()}-${pad(ahora.getMonth() + 1)}-${pad(ahora.getDate())}T${pad(ahora.getHours())}:${pad(ahora.getMinutes())}:${pad(ahora.getSeconds())}-04:00`;

  const payload = {
    rut: usuarioLogeado.rut ?? '11879840-6',
    otp: codigoOTP,
    scheduleId: horario.scheduleId,
    date: fechaFormateada,
    organizationPersonRoleId: usuarioLogeado.organizationPersonRoleId ?? 1061,
    comment: comentarioFirma,
    classObservation: observacionClase,
    classType: tipoClase
  };
  ...
  try {
    const res = await axios.post('http://localhost:5000/api/calendariohorarios/firmar-bloque', payload);

    if (res.data.success && res.data.otpVerified) {
      ...
      setComentarioFirma('');
      setCodigoOTP('');
      setObservacionClase(''); // <== este faltaba
    } else if (res.data.success && res.data.otpVerified === false) {
      alert('⚠️ Código OTP incorrecto. No se registró la firma.');
    } else {
      alert('⚠️ No se pudo registrar la firma: ' + res.data.message);
    }
  } catch (err) {
    alert('❌ Error al firmar el bloque:\n' + (err.response?.data?.error || 'Error desconocido'));
  }
...
const cargarHistorialFirmas = async () => {
  console.log("🔁 Cargando historial de firmas...");
  try {
    const res = await axios.get('http://localhost:5000/api/calendariohorarios/historial-firmas', {
      params: {
        scheduleId: horario.scheduleId
      }
    });
    console.log("✅ Respuesta del historial:", res.data);
    setHistorialFirmas(res.data.firmas || []);
  } catch (err) {
    console.error("❌ Error al cargar historial de firmas:", err);
    setHistorialFirmas([]);
  }
};
...
const usuario = {
  nombre: usuarioLogeado?.nombre || usuarioLogeado?.Nombre || "Usuario de Prueba",
  rut: usuarioLogeado?.rut || "11879840-9"
};

useEffect(() => {
  if (tabIndex === 4) {
    cargarHistorialFirmas(); // ✅ se dispara al abrir la pestaña de firma digital
  }
}, [tabIndex, horario?.scheduleId]);


// ============================ RFICA2 FIN ==============================


// Ya no se cargan observaciones previas en el formulario de ingreso
useEffect(() => {
  if (tabIndex === 3 && tabObservacion === 0) {
    setObservacionCurso('');
    setTipoCurso('');
    setArchivoCurso(null);
    setMensajeObs('');
  }
}, [tabIndex, tabObservacion]);



/* RFICA1   USESTATE PARA INGRESO OBSERVACIONES POR ESTUDIANTE  */

// 🔹 Observación Estudiantes
const [observacionEstudianteTexto, setObservacionEstudianteTexto] = useState('');
const [tipoObservacionEstudiante, setTipoObservacionEstudiante] = useState('');
const [estudiantesSeleccionados, setEstudiantesSeleccionados] = useState([]);
const [archivoEstudiante, setArchivoEstudiante] = useState(null);
...
//**************************+FIN DE MANEJO DE MODAL INGRESO DE OAP
  ...
  if (!cursoId || cursoId === 'No disponible') {
    Swal.fire('❌ Error', 'El ID del curso no está definido correctamente.', 'error');
    return;
  }

  if (!observacionCurso || !tipoCurso) {
    Swal.fire('❌ Faltan datos', 'Debe escribir una observación y clasificarla.', 'warning');
    return;
  }
  ...
  const userId = usuarioLogeado?.PersonId ?? '1061';
  formData.append('usuarioId', userId);

  if (archivoCurso) {
    formData.append('archivo', archivoCurso);
  }
  ...
  try {
    ...
    Swal.fire('✅ Observación guardada correctamente');
    // 🔄 Limpiar campos
    setObservacionCurso('');
    setTipoCurso('');
    setArchivoCurso(null);

    await cargarObservacionesDesdeDB(); // ✅ Refrescar observaciones ingresadas
  } catch (error) {
...
/*RFICA1  GUARDA OBSERVACIONES DE ESTUDIANTES****************************************/

const handleGuardarObservacionEstudiantes = async () => {
  if (
    !observacionEstudianteTexto ||
  ...
  const formData = new FormData();
  formData.append("observacion", observacionEstudianteTexto);
  formData.append("tipoObservacion", tipoObservacionEstudiante);
  formData.append("relacionadoCon", relacionadoCon || '');
  formData.append("usuarioId", usuarioId); // ya lo tienes en el frontend
  formData.append("fecha", new Date().toISOString().split('T')[0]);
  formData.append("estudiantesSeleccionados", JSON.stringify(estudiantesSeleccionados));
  if (archivoEstudiante) formData.append("archivo", archivoEstudiante);

  try {
    await axios.post("http://localhost:5000/api/calendariohorarios/guardar-observacion-estudiantes", formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });

	Swal.fire("✅ Observaciones guardadas");

    setObservacionEstudianteTexto('');
    setTipoObservacionEstudiante('');
    setEstudiantesSeleccionados([]);
    setArchivoEstudiante(null);
    setRelacionadoCon('');
  } catch (error) {
...
const usuarioId = 2095; // ← Agrega esto
  ...
  /*RFICA1FIN  GUARDA OBSERVACIONES DE ESTUDIANTES****************************************/
...
useEffect(() => {
  if (tabIndex === 3 && tabObservacion === 2 && horario?.scheduleId) {
...
/** RFICA1 USEEFFECT PARA OBSERVACIONES PARA ESTUDIANTES */

useEffect(() => {
  const obtenerEstudiantes = async () => {
    try {
      const resp = await axios.get(`http://localhost:5000/api/calendariohorarios/estudiantes/${selectedCursoId}`);
  ...
  if (selectedCursoId && tabIndex === 3 && tabObservacion === 1) {
    obtenerEstudiantes();
  }
}, [selectedCursoId, tabIndex, tabObservacion]);

/** RFICA1FIN USEEFFECT PARA OBSERVACIONES PARA ESTUDIANTES */
...
// ============================
//  Funciones para acciones de Observaciones Ingresadas
// ============================
// 🔄 RFICA1: Eliminar observación (curso o estudiante)
const handleEliminarObservacion = (id, tipo) => {
  Swal.fire({
    title: '¿Estás seguro?',
    text: "Esta acción eliminará la observación.",
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (tipo === 'Estudiante') {
          await axios.delete(`http://localhost:5000/api/calendariohorarios/observacion-estudiante/${id}`);
        } else {
          await axios.delete(`http://localhost:5000/api/calendariohorarios/observacion-curso/${id}`);
        }

        Swal.fire('✅ Eliminado', 'La observación ha sido eliminada.', 'success');
        await cargarObservacionesDesdeDB();
      } catch (error) {
...
const handleVerObservacion = (obs) => {
  const tipoTexto = {
    1: 'Positiva',
    2: 'Negativa',
    3: 'Neutra'
  ...
  Swal.fire({
    title: '📝 Observación',
    html: `
      <p><strong>Docente:</strong> ${obs.NombreDocente || "-"}</p>     
      <p><strong>Tipo:</strong> ${tipoTexto}</p>
      <p><strong>Fecha:</strong> ${obs.Fecha ? obs.Fecha.substring(0, 10) : '-'}</p>
      <p><strong>Contenido:</strong><br>${obs.Texto}</p>
    `,
...
const cargarObservacionesDesdeDB = async () => {
  try {
    const resp = await axios.get(`http://localhost:5000/api/calendariohorarios/observaciones/bloque/${horario.scheduleId}`);
    setObservacionesIngresadas(resp.data);
	console.log("🔍 Datos cargados:", resp.data);

  } catch (error) {
    console.error('Error al recargar observaciones después de edición:', error);
  }
};
const handleEditarObservacion = (obs) => {
  Swal.fire({
    title: 'Editar Observación',
    html: `
      <div style="text-align:left; width:100%;">
        <textarea id="textoObservacion" class="swal2-textarea" 
          style="width:100%; height:100px; resize:vertical; font-size:14px;">${obs.Texto}</textarea>
        <br/>
        <label style="display:block; margin-top:10px;"><b>Tipo de Observación:</b></label>
        <div style="display:flex; gap:15px; flex-wrap:wrap;">
          <label><input type="radio" name="tipoObs" value="1" ${obs.RefObservationTypeId === 1 ? 'checked' : ''}/> Positiva</label>
          <label><input type="radio" name="tipoObs" value="2" ${obs.RefObservationTypeId === 2 ? 'checked' : ''}/> Negativa</label>
          <label><input type="radio" name="tipoObs" value="3" ${obs.RefObservationTypeId === 3 ? 'checked' : ''}/> Neutra</label>
        </div>
      </div>
    `,
    customClass: {
      popup: 'ancho-modal-observacion'
    },
    showCancelButton: true,
    focusConfirm: false,
    ...
    cancelButtonText: 'Cancelar',
    preConfirm: async () => {
      const nuevoTexto = document.getElementById('textoObservacion').value;
      const tipoSeleccionado = document.querySelector('input[name="tipoObs"]:checked').value;

      try {
        if (obs.Tipo === 'Estudiante') {
          await axios.put(
            `http://localhost:5000/api/calendariohorarios/observaciones/estudiante/${obs.Id}`,
            {
              texto: nuevoTexto,
              RefObservationTypeId: parseInt(tipoSeleccionado)
            }
          );
        } else {
          await axios.put(
            `http://localhost:5000/api/calendariohorarios/observaciones/curso/${obs.Id}`,
            {
              texto: nuevoTexto,
              RefObservationTypeId: parseInt(tipoSeleccionado)
            }
        ...
        Swal.fire('✅ Observación actualizada.', '', 'success');
        cargarObservacionesDesdeDB();
      } catch (error) {
...
{/* ============================RFICA1 Observación Estudiantes ============================ */}
{tabObservacion === 1 && (
  <Grid container spacing={2} style={{ marginTop: 10 }}>

    {/* Columna 1: Observación */}
    <Grid item xs={12} md={4}>
      <Typography><b>Observación del estudiante</b></Typography>
      <TextField
        fullWidth
        multiline
        rows={6}
        variant="outlined"
        value={observacionEstudianteTexto}
        onChange={(e) => setObservacionEstudianteTexto(e.target.value)}
      />
    </Grid>

    {/* Columna 2: Autocomplete de estudiantes */}
    <Grid item xs={12} md={4}>
      <Typography><b>Seleccione estudiante(s)</b></Typography>
      <Autocomplete
        multiple
        options={estudiantes}
        getOptionLabel={(option) => option.NombreCompleto}
        value={estudiantes.filter(e => estudiantesSeleccionados.includes(e.OrganizationPersonRoleId))}
        onChange={(event, newValue) =>
          setEstudiantesSeleccionados(newValue.map(est => est.OrganizationPersonRoleId))
        }
        filterSelectedOptions
        renderInput={(params) => (
          <TextField {...params} variant="outlined" placeholder="Seleccione..." />
        )}
      />
    </Grid>

    {/* Columna 3: Archivo, tipo, guardar */}
    <Grid item xs={12} md={4}>
      <Typography><b>Adjunte archivo</b></Typography>
      <input type="file" onChange={(e) => setArchivoEstudiante(e.target.files[0])} />

      <Typography style={{ marginTop: 16 }}><b>Clasifique su observación</b></Typography>
      <RadioGroup
        value={tipoObservacionEstudiante}
        onChange={(e) => setTipoObservacionEstudiante(e.target.value)}
      >
        <FormControlLabel value="1" control={<Radio />} label="Positiva" />
        <FormControlLabel value="2" control={<Radio />} label="Negativa" />
        <FormControlLabel value="3" control={<Radio />} label="Neutra" />
      ...
      <Button
        ...
        color="primary"
        style={{ marginTop: 16 }}
        startIcon={<i className="fas fa-save" />} // si tienes fontawesome
        onClick={handleGuardarObservacionEstudiantes}
      >
        Guardar
      </Button>
    </Grid>
...
{/* ============================FIN RFICA1 Observación Estudiantes ============================ */}


{/* ============================ RFICA1 Observación INGRESADAS ============================ */}
			...
			<Table>
			  <TableHead>
				<TableRow>
				  ...
				  <TableCell><b>General/Estudiante</b></TableCell>
			  ...
			  <TableBody>
				{observacionesIngresadas.map((obs, idx) => (
				 <TableRow key={idx}>
					  <TableCell>{(obs.NombreDocente && obs.NombreDocente.trim()) ? obs.NombreDocente : '—'}</TableCell>					
					  <TableCell>{obs.Tipo === 'General' ? 'General' : (obs.NombreEstudiante || 'Estudiante')}</TableCell>
					  ...
					  {/* Nueva celda para ícono de archivo */}
					  <TableCell>
						{obs.FileUrl && (
						  <a				
						href={`http://localhost:5000/api/calendariohorarios/descargar-archivo/${encodeURIComponent(obs.FileUrl.replace(/^archivo\//, ''))}`}
							target="_blank"
							rel="noopener noreferrer"
							title="Ver archivo adjunto"
						  >
							<FaFileAlt style={{ color: 'green', cursor: 'pointer' }} />
						  </a>
						)}
					  </TableCell>
					  ...
					  <TableCell>
						...
						>
						  <FaEye style={{ color: 'dodgerblue' }} />
						</button>
						<button
						  className="btn-icon"
						  title="Editar"
						  onClick={() => handleEditarObservacion(obs)}
						>
						...
						<button
						  ...
						  onClick={() => handleEliminarObservacion(obs.Id, obs.Tipo)}
	...
	{/* ============================ RFICA1FIN Observación INGRESADAS ============================ */}		
...
{/* ============================ RFICA2INICIO Firma digital profesor ============================ */}

{tabIndex === 4 && (
  <Paper elevation={3} style={{ marginTop: '20px', padding: '20px' }}>
    <div style={{ background: '#ffeaea', padding: 10, borderRadius: 4, marginBottom: 20 }}>
      Al ingresar su firma, usted está declarando que los atributos ingresados en este bloque fueron realizados con el estándar de datos requeridos por el Mineduc.
    </div>

    <div className="firma-bloque-contenedor">
      <div className="bloque-usuario-caja">
        <div><b>Usuario:</b> {usuario.nombre || '—'}</div>
        <div><b>RUT:</b> {usuario.rut || '—'}</div>
        <div><label><input type="checkbox" /> Firma otro usuario</label></div>
      </div>

      <TextField
        label="Comentario adjunto a la firma digital"
        multiline
        minRows={3}
        className="bloque-textarea-firma"
        value={comentarioFirma}
        onChange={e => setComentarioFirma(e.target.value)}
      />

      <div className="bloque-otp-firma">
        <label style={{ fontSize: 13, marginBottom: 4 }}>Ingrese el código verificador para firmar este bloque:</label>
    ...
    <div style={{ margin: '18px 0', background: '#f5faff', padding: 10, borderRadius: 4 }}>
      Yo {usuario.nombre || '—'} dejo constancia que estos atributos corresponden a este bloque y así firmo el libro de clases digital.
    </div>

    <Grid container spacing={2} style={{ marginTop: 10 }}>
      <Grid item xs={12} md={8}>
        <TextField
          label="Observación de la clase"
          multiline
          fullWidth
          minRows={4}
          maxRows={6}
          ...
          className="observacion-clase-ajustada"
        />
      </Grid>
      <Grid item xs={12} md={4}>
        <div className="bloque-tipo-clase">
          <div><b>Tipo de clase</b></div>
          <div><label><input type="radio" name="tipoClase" value="1" checked={tipoClase==='1'} onChange={()=>setTipoClase('1')} /> Regular</label></div>
          <div><label><input type="radio" name="tipoClase" value="2" checked={tipoClase==='2'} onChange={()=>setTipoClase('2')} /> Suspendida</label></div>
          <div><label><input type="radio" name="tipoClase" value="3" checked={tipoClase==='3'} onChange={()=>setTipoClase('3')} /> Recuperativa</label></div>
        </div>
      </Grid>
    </Grid>
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell><b>Usuario que firmó</b></TableCell>
              <TableCell><b>Comentario</b></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {historialFirmas.map((f, idx) => (
              <TableRow key={idx}>
                <TableCell>{f.fechaHora}</TableCell>
                <TableCell>{f.otpVerified ? '✔️ Válida' : '❌ Inválida'}</TableCell>
                <TableCell>{f.usuarioNombre}</TableCell>
                <TableCell>{f.comentario}</TableCell>
              </TableRow>
            ))}
            {historialFirmas.length === 0 && (
              <TableRow><TableCell colSpan={4} align="center">Sin firmas registradas</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </TableContainer>
...
{/* ============================ RFICA2FIN Firma digital profesor ============================ */}
...
const CalendarioHorarios = () => {
  console.log('[DEBUG] CalendarioHorarios montado');

	   const usuarioLogeado = RoleService.getUsuario();
  ...
  // useEffect para setear colegio automáticamente SOLO cuando colegios estén cargados
  useEffect(() => {
    const usuario = RoleService.getUsuario();
    console.log('[DEBUG] Usuario:', usuario);
    console.log('[DEBUG] Colegios cargados:', colegios);
    if (
      usuario &&
      (usuario.RoleId === 11 || usuario.RoleId === 13) &&
      usuario.SchoolId &&
      colegios.length > 0 &&
      colegios.some(c => String(c.OrganizationId) === String(usuario.SchoolId)) &&
      !selectedColegio // Solo si aún no está seleccionado
    ) {
      console.log('[DEBUG] Seteando colegio automáticamente:', usuario.SchoolId);
      setSelectedColegio(String(usuario.SchoolId));
      setColegioDisabled(true);
      fetchCursos(String(usuario.SchoolId));
    } else if (!usuario || (usuario.RoleId !== 11 && usuario.RoleId !== 13)) {
  ...
  const fetchColegios = async () => {
    try {
      ...
      console.log('[DEBUG] Colegios recibidos en fetchColegios:', response.data);
    } catch (error) {
