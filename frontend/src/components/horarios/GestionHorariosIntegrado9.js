...
// NUEVO método seguro de toggle
const toggleAsistencia = async (dia, index) => {
  const bloqueSeleccionado = horarios[dia][index];

  // Validar si estamos en modo inserción
  const tieneDatosReales = bloqueSeleccionado?.asignatura && bloqueSeleccionado?.horaInicio && bloqueSeleccionado?.horaTermino && bloqueSeleccionado?.bloqueId && calendarioSeleccionado;
  if (!tieneDatosReales) {
    alert("Primero debes guardar los horarios antes de marcar bloques de asistencia.");
    return;
  }

  // Si ya está marcado, simplemente desmarcar
  if (bloqueSeleccionado.asistencia === true) {
    try {
      const res = await axios.post("http://localhost:5000/api/horarios/actualizar-asistencia", {
        organizationId: bloqueSeleccionado.asignatura,
        classMeetingDays: dia,
        classBeginningTime: bloqueSeleccionado.horaInicio,
        classEndingTime: bloqueSeleccionado.horaTermino,
        organizationCalendarSessionId: calendarioSeleccionado,
        asistencia: false,
      });
      ...
      setHorarios(prev => {
        const copia = { ...prev };
        copia[dia][index].asistencia = false;
        return copia;
      });

      alert("❌ Bloque desmarcado de asistencia");
    } catch (error) {
      console.error("❌ Error al desmarcar asistencia:", error);
      alert("Error al actualizar la asistencia del bloque.");
    }
    return;
  }

  // Validar si ya hay otro bloque marcado en ese mismo día
  const yaMarcado = horarios[dia].findIndex((b, i) => b.asistencia === true && i !== index);
  if (yaMarcado !== -1) {
    alert("Ya existe un bloque marcado para asistencia en este día. Solo puede haber uno.");
    return;
  }

  // Marcar este nuevo bloque
  try {
    const res = await axios.post("http://localhost:5000/api/horarios/actualizar-asistencia", {
      organizationId: bloqueSeleccionado.asignatura,
      classMeetingDays: dia,
    ...
    setHorarios(prev => {
      const copia = { ...prev };
      copia[dia][index].asistencia = true;
      return copia;
    });

    alert("✔️ Bloque marcado para asistencia");
  } catch (error) {
...
// AJUSTE para evitar alertas confusas
const marcarAsistenciaEnBD = async (dia, bloque, nuevoEstado) => {
  try {
    await axios.post("http://localhost:5000/api/horarios/actualizar-asistencia", {
    ...
    alert("Ocurrió un error al actualizar la asistencia del bloque.");
  }
};
...
if (!bloqueCoincidente) {
  ...
  console.log("✅ Bloque encontrado:", bloqueCoincidente);
     ...
     const bloqueId = bloqueCoincidente ? Number(bloqueCoincidente.id) : null;
      ...
      nuevosHorarios[hor.dias].push({
        scheduleId: hor.id, // <--- ID real de BD
        horaInicio,
        horaTermino: horaFin,
        asignatura: asigId,
        profesor: profId,
  ...
  organizationId: bloque.asignatura ? Number(bloque.asignatura) : null,
...
// Actualizar los IDs recién generados en el estado
