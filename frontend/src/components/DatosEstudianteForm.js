...
const DatosEstudianteForm = () => {
  ...
  // NUEVO: Estados para RUN/IPE del apoderado
  const [rutOipeApoderadoTipo, setRutOipeApoderadoTipo] = useState('RUN');
  const [rutOipeApoderado, setRutOipeApoderado] = useState('');
  ...
  useEffect(() => {
    if (selectedColegio) { // Verifica que selectedColegio no sea null, undefined o 0
      // Obtener niveles por colegio usando el endpoint de horarios
      axios.get(`http://localhost:5000/api/horarios/codense/${selectedColegio}`)
        .then((response) => {
          const formattedNiveles = response.data.map((nivel) => ({
            OrganizationId: String(nivel.OrganizationId),
            Name: nivel.Name,
          }));
  ...
  // useEffect para cargar cursos SOLO cuando cambia el nivel seleccionado
  useEffect(() => {
    if (nivel && selectedColegio) {
      axios.get(`http://localhost:5000/api/horarios/cursos/${nivel}`)
        .then((response) => {
          setCursoOptions(response.data || []);
          setCurso(''); // Resetear el curso seleccionado cuando cambia el nivel
        })
  ...
  // Obtener información del usuario actual usando RoleService
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Extraer solo el ID numérico del curso (después del guion)
    let cursoIdSolo = curso;
    if (typeof curso === 'string' && curso.includes('-')) {
      const partes = curso.split('-');
      cursoIdSolo = partes[1] ? partes[1].trim() : curso;
    }
    formData.append('curso', cursoIdSolo);
    ...
    if (viveCon === 'Apoderado') {
      ...
      formData.append('rutOipeApoderadoTipo', rutOipeApoderadoTipo);
      formData.append('rutOipeApoderado', rutOipeApoderado);
    }

    formData.append('selectedColegio', selectedColegio);
  ...
  console.log('cursoOptions:', cursoOptions);

  // Justo antes del return, obtengo la parte textual relevante del nivel seleccionado
  const nivelSeleccionadoObj = nivelOptions.find(n => n.OrganizationId === nivel);
  let textoNivel = '';
  if (nivelSeleccionadoObj) {
    // Extrae la parte textual después del primer guion
    const partes = nivelSeleccionadoObj.Name.split(' - ');
    textoNivel = partes.length > 1 ? partes[1].trim().split(' ')[0] : '';
  }

  // LOGS DE DEPURACIÓN PARA EL SELECT DE CURSO
  useEffect(() => {
    console.log('--- DEPURACIÓN SELECT CURSO ---');
    console.log('Valor actual de curso:', curso, typeof curso);
    console.log('Opciones de curso:', cursoOptions.map(opt => String(opt.Valor)));
    if (curso && !cursoOptions.some(opt => String(opt.Valor) === String(curso))) {
      console.log('El valor de curso no está en las opciones, reseteando a vacío');
      setCurso('');
    }
  }, [curso, cursoOptions]);
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <FormControl fullWidth required variant="outlined" className={classes.formControl}>
            ...
            <Select
              value={selectedColegio ? String(selectedColegio) : ''}
              onChange={(e) => setSelectedColegio(e.target.value)}
              className={classes.selectField}
              label="Colegio"
              disabled={RoleService.esRol(11) || RoleService.esRol(13)}
            >
              {colegios.map((colegio) => (
                <MenuItem key={colegio.OrganizationId} value={String(colegio.OrganizationId)}>
                  {colegio.Name}
                </MenuItem>
              ))}
            </Select>
          ...
          <FormControl fullWidth required variant="outlined" className={classes.formControl}>
            ...
            >
              {nivelOptions.map((option) => (
                <MenuItem key={option.OrganizationId} value={String(option.OrganizationId)}>
                  {option.Name}
                </MenuItem>
              ))}
            </Select>
        ...
        <Grid item xs={12} sm={6}>
          <FormControl fullWidth required variant="outlined" className={classes.formControl}>
            ...
            <Select
              ...
              className={classes.selectField}
              label="Curso"
              disabled={cursoOptions.length === 0}
            >
        ...
        {viveCon === 'Apoderado' && (
          <>
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth required variant="outlined" className={classes.formControl}>
                <InputLabel>RUT o IPE Apoderado</InputLabel>
                <Select
                  value={rutOipeApoderadoTipo}
                  onChange={e => setRutOipeApoderadoTipo(e.target.value)}
                  label="RUT o IPE Apoderado"
                >
                  <MenuItem value="RUN">RUN</MenuItem>
                  <MenuItem value="IPE">IPE</MenuItem>
                </Select>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                label={rutOipeApoderadoTipo === 'RUN' ? 'RUN Apoderado' : 'IPE Apoderado'}
                value={rutOipeApoderado}
                onChange={e => setRutOipeApoderado(e.target.value)}
                required
                fullWidth
                variant="outlined"
                className={classes.inputField}
